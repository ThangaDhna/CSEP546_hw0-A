...
def crossentropy_parameter_search(
    ...
    architectures = [
        ("Linear",
         nn.Sequential(LinearLayer(2, 2, RNG), SoftmaxLayer())),
        ("1-hidden-Sigmoid",
         nn.Sequential(LinearLayer(2, 2, RNG), SigmoidLayer(), LinearLayer(2, 2, RNG), SoftmaxLayer())),
        ("1-hidden-ReLU",
         nn.Sequential(LinearLayer(2, 2, RNG), ReLULayer(), LinearLayer(2, 2, RNG), SoftmaxLayer())),
        ("2-hidden-Sigmoid-ReLU",
         nn.Sequential(LinearLayer(2, 2, RNG), SigmoidLayer(), LinearLayer(2, 2, RNG), ReLULayer(), LinearLayer(2, 2, RNG), SoftmaxLayer())),
        ("2-hidden-ReLU-Sigmoid",
         nn.Sequential(LinearLayer(2, 2, RNG), ReLULayer(), LinearLayer(2, 2, RNG), SigmoidLayer(), LinearLayer(2, 2, RNG), SoftmaxLayer())),
        ("2-hidden-ReLU-ReLU",
         nn.Sequential(LinearLayer(2, 2, RNG), ReLULayer(), LinearLayer(2, 2, RNG), ReLULayer(), LinearLayer(2, 2, RNG), SoftmaxLayer())),
    ]
...
def main():
    ...
    for (name, data), color in zip(ce_configs.items(), colors):
    ...
    plt.title("Cross-Entropy Loss — Training and Validation (all 6 architectures)")
    plt.legend(fontsize=7, ncol=2)
    plt.tight_layout()
    plt.savefig(os.path.join(_HW3_DIR, "a4_ce_loss.png"), dpi=150)
    plt.close()
    print("Saved a4_ce_loss.png")

    # Best model by lowest validation loss at any epoch
    best_name = min(ce_configs.keys(), key=lambda n: min(ce_configs[n]["val"]))
    best_model = ce_configs[best_name]["model"]
    best_val_loss = min(ce_configs[best_name]["val"])
    print(f"Best CE model: {best_name}, best val loss: {best_val_loss:.4f}")

    test_loader = DataLoader(dataset_test, batch_size=len(dataset_test))
    test_acc = accuracy_score(best_model, test_loader)
    print(f"CE best model test accuracy: {test_acc:.4f}")

    # Scatter plot of predictions on test set
    fig = plt.figure(figsize=(7, 6))
    plot_model_guesses(test_loader, best_model, title=f"CE Best: {best_name}\nTest Accuracy = {test_acc:.4f}")
    fig.savefig(os.path.join(_HW3_DIR, "a4_ce_predictions.png"), dpi=150)
    plt.close("all")
    print("Saved a4_ce_predictions.png")
