...
def vanilla_solution(x: Vector, y: Vector, A: Matrix, B: Matrix) -> Vector:
    ...
    """
    AT = vanilla_transpose(A)
    BT = vanilla_transpose(B)
    n = len(x)
    AplusAT = [[A[i][j] + AT[i][j] for j in range(n)] for i in range(n)]
    term1 = vanilla_matmul(AplusAT, x)
    term2 = vanilla_matmul(BT, y)
    return [term1[i] + term2[i] for i in range(n)]
...
def main():
    ...
    plt.ylabel("Time (ns)")
    plt.legend()
    plt.savefig("a7c_timing.png", dpi=150, bbox_inches="tight")
    plt.show()
